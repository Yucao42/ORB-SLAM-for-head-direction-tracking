...
cv::Mat rot2euler(const cv::Mat & rotationMatrix)
{
  ...
  if (m10 > 0.998) { // singularity at north pole
    x = 0;
    y = 90;
    z = atan2(m02,m22)*180.0/cv::CV_PI;
  }
  ...
  {
    x = atan2(-m12,m11)*180.0/cv::CV_PI;
    y = asin(m10)*180.0/cv::CV_PI;
    z = atan2(-m20,m00)*180.0/cv::CV_PI;
  }

  euler.at<double>(0) = x;
  euler.at<double>(1) = y;
  euler.at<double>(2) = z;
        cout<<"Yaw angle: "<<x<<", Pitch angle: "<<y<<", Roll angle: "<<z<<"."<<endl;
...
{
    Eigen::Matrix<double,3,3> eigMat = Converter::toMatrix3d(M);
...
//    Quaterniond r(mCurrentFrame.GetRotationInverse());
//    cout<<"Rotation angle: "<<acos(r(0))*180.00/CV_PI<<endl;
    //cout<<"Eular angles: "<<rot2euler(mCurrentFrame.GetRotationInverse())*180.00/CV_PI<<endl;
    dcm2euler(mCurrentFrame.GetRotationInverse().t());
